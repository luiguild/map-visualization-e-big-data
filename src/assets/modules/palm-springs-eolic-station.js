...
const run = () => {
    ...
    const Extent = arceasy.obj.constructors.utils.Extent
    ...
    ******************************************************/
    function getWindDirection () {
        var layerURL = 'https://services.arcgis.com/V6ZHFr6zdgNZuVG0/arcgis/rest/services/weather_stations_010417/FeatureServer/0/query'
    ...
    ****************************************************/
    all([getWindDirection(), getWeatherStations()])
    .then(function (results) {
        var wind = results[0]
        var stations = results[1]
        var myExternalRenderer = new MyExternalRenderer(view, wind, stations)
    ...
    var MyExternalRenderer = declare(null, {
        ...
        */
        initData: function (context, wind, stations) {
            ...
            this.localOriginRender = externalRenderers.toRenderCoordinates(view, this.localOrigin, 0, localOriginSR, new Float32Array(3), 0, 1)
            ...
            for (var i = 0; i < this.numStations; ++i) {
                ...
                var point = [station.geometry.x, station.geometry.y, station.getAttribute('POINT_Z') || station.geometry.z]
                var inputToRender = externalRenderers.renderCoordinateTransformAt(view, point, inputSR, new Float64Array(16))
            ...
            for (let i = 0; i < this.numStations; ++i) {
                ...
                for (let j = 0; j < numCoordinates; ++j) {
                    this.windmillBasePositions[i * numCoordinates + j] = windmill_base_positions[j] * this.windmillInstanceTowerScale[i]
                    this.windmillBaseNormals[i * numCoordinates + j] = windmill_base_normals[j]
                }

                // Transform vertices into render coordinates
                glMatrix.vec3.forEach(this.windmillBasePositions, 0, i * numCoordinates, numVertices, glMatrix.vec3.transformMat4, positionMatrix)

                // Subtract local origin coordinates
                glMatrix.vec3.forEach(this.windmillBasePositions, 0, i * numCoordinates, numVertices, glMatrix.vec3.subtract, this.localOriginRender)

                // Transform normals into render coordinates
                glMatrix.vec3.forEach(this.windmillBaseNormals, 0, i * numCoordinates, numVertices, glMatrix.vec3.transformMat3, normalMatrix)

                // Re-normalize normals
                glMatrix.vec3.forEach(this.windmillBaseNormals, 0, i * numCoordinates, numVertices, glMatrix.vec3.normalize)
        ...
        */
        setCommonUniforms: function (context) {
            ...
            gl.uniform3fv(this.programUniformDirectionalColor, this.getFlatColor(context.sunLight.diffuse, this.tempVec3))
